...
For every nonloop $e\in M$, order
$N_B(e)=(x_{e,1},\ldots,x_{e,d_e})$. Replace $e$ by
...
\]
A source loop is retained as a one-element loop and its chain has effective
length one.

\begin{lemma}[Private-chain matching]
For every nonloop $e$, every proper subset of $C_e$ is matchable into $P_e$.
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
If $A$ contains a loop, neither side is matchable. Otherwise, a source matching
edge $e x_{e,j}$ is lifted by matching $e_j$ externally and the remaining chain
copies privately. Conversely, a complete nonloop chain $C_e$ has $d_e$ left
...
$w(e)=\max\{1,d_e\}$, where the value one handles loops. If all source
elements have the same positive degree $D$, then
\[
...
\end{theorem}
\begin{proof}
A circuit of $B'$ cannot meet a nonloop chain in a nonempty proper subset: that
part has a private matching, so deleting it would leave a dependent proper
subset, contradicting minimality. Thus every circuit is a union of complete
chains; retained loops already satisfy the claim. Complete-chain dependence
and minimality are equivalent to source dependence and minimality by the
preceding lemma. The girth statements follow.
\end{proof}
...
$\ell={k\choose2}-k-1={k-1\choose2}$. We do not use that identity. Instead we
rederive the reservoir size required by Hall deficiency.
...
left-regular, loopless Clique presentation. Its maximum left degree becomes
three and its girth is multiplied by $D$. Consequently,
\[
