...
\begin{lemma}[Polynomial Hall-circuit extraction]
The output-support bipartite graph defines a transversal matroid on outputs.
When $m>n$, compute a maximum support matching with matched-output basis $I$ and
choose an unmatched output $e$. The fundamental circuit
\[
 F=\{e\}\cup\{f\in I: I-f+e\text{ is matchable}\}
\]
is computable with polynomially many bipartite-matching tests. It satisfies
\[
 |F|=|N(F)|+1,
\]
...
\begin{proof}
A fundamental circuit is minimally dependent in the transversal matroid. Hence
every proper subset is matchable. Hall's theorem says the full set is the only
possible Hall violation. A matching of $|F|-1$ outputs exists, so
$|N(F)|\ge |F|-1$, while dependence gives $|N(F)|<|F|$; equality follows.
\end{proof}
...
triangle. The Hall-circuit inequalities therefore satisfy Rado's
...
\begin{proof}
Extract a fundamental Hall circuit by maximum matching and omit one gate.
Construct the frame-independent pair transversal. If the omitted support meets
multiple components, one of its pair clauses joins two unbalanced cycles and
yields the V105 loose-handcuff contradiction. Otherwise compress the common
component to the constant kernel, find an unsatisfiable phase/target assignment
by the finite lemma, and lift the virtual clauses to exact original majority
clauses. Extra clauses supplied by each fixed majority output only strengthen
the resulting 2-CNF formula. Thus the chosen block output has no preimage, and
arbitrary values outside the block extend it to a missing full output. Maximum
matching, fundamental-circuit extraction, matroid intersection, and graph
reduction are polynomial, while the kernel search has constant size.
\end{proof}
