...
## Lemma 1 — polynomial minimal-surplus extraction

From any output family with `m>n`, an inclusion-minimal output subfamily `F` with

```text
|F| > |N(F)|
```

can be found by greedily deleting any output whose deletion preserves positive
surplus.  Minimality forces

```text
|F| = |N(F)| + 1,
```

and every proper `J subset F` satisfies Hall:
...
1. Extract an inclusion-minimal positive-surplus output subfamily `F` (Lemma 1).
...
All stages are polynomial: surplus extraction is explicit set processing,
matroid intersection uses polynomial independence oracles, the graph reductions
are linear/polynomial, and the kernel search is constant-size.
...
Novelty is not confirmed.  The proof uses standard Rado/matroid intersection,
signed-frame matroids, 2-SAT SCCs, and a new-to-this-laboratory constant kernel
composition that requires external specialist review.
