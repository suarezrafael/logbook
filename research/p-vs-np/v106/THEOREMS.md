...
positive surplus, the full family is necessarily beyond the rank budget; the
unresolved issue is how to force the resulting colorful frame dependence to be
an odd handcuff rather than a balanced cycle.
...
## Lemma 6 — the strict family is minimally positive-surplus

The full family has `m=n+1`. Every proper output subfamily satisfies Hall.

### Proof

It is enough to show that after deleting any one output, the remaining `n`
outputs have a system of distinct representatives on the `n` inputs. Any proper
subfamily is contained in one of these single-deletion families and therefore
inherits Hall.

Write the subdivided path as

```text
v0=0, v1, ..., v_{2q}, v_{2q+1}=2
```

with path gate `P_i` supported on `{v_i,v_{i+1},1}`.

If a path gate `P_j` is deleted, match the five base gates bijectively to
`{0,1,2,3,4}` by

```text
G0->0, G1->3, G2->4, G3->2, G4->1.
```

For `i<j`, match `P_i->v_{i+1}`; for `i>j`, match `P_i->v_i`. This matches all
remaining path gates to the `2q` internal path vertices.

If a base gate is deleted, match all path gates by

```text
P0->0,  P_i->v_i for i>=1,
```

which uses input `0` and all internal path vertices. The four surviving base
gates are then matched to `{1,2,3,4}` by the following table:

```text
delete G0: G1->3, G2->4, G3->2, G4->1
delete G1: G0->1, G2->4, G3->2, G4->3
delete G2: G0->1, G1->4, G3->2, G4->3
delete G3: G0->1, G1->4, G2->2, G4->3
delete G4: G0->1, G1->3, G2->4, G3->2
```

Every assignment uses a variable contained in the corresponding support and is
injective. Thus every single-deletion family has a perfect support matching.
...
while V106 has `sigma=1`. The construction is also switching-unbalanced, and by
Lemma 6 the full output set is the first positive-surplus set.
