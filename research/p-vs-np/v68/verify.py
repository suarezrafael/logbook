...
def verify_repository_surface(results):
    required=['README.md','SPINE_FAMILY_THEOREM.md','V68_SPINE_TREE_DAG_THEOREM.tex','C36_STRUCTURE_ANALYSIS.md','PROJECTED_DAG_MODEL.md','BITSET_ENGINE.md','PROOF_COMPLEXITY_BOUNDARY.md','RESULTS.json','V69_CORE_CONTEXT.md','affine_bitset.py','v68_spine_family.py','verify_independent.py']
    ...
    runner=(ROOT/'verify_all.sh').read_text();assert 'V68|primary|v68/verify.py|quick|' in runner and 'V68|independent|v68/verify_independent.py|quick|' in runner
    state=(ROOT/'STATE.md').read_text();current=re.search(r'\*\*Current laboratory:\*\* V(\d+)',state)
    assert current and int(current.group(1))>=68 and 'Direct P-versus-NP route active:** no' in state and 'G_proj' in state
    root=(ROOT/'README.md').read_text();assert '[`v68/`](v68/)' in root
    workflow=(ROOT.parent.parent/'.github'/'workflows'/'p-vs-np-verify.yml').read_text();assert 'V68_SPINE_TREE_DAG_THEOREM.tex' in workflow
    theorem=(HERE/'SPINE_FAMILY_THEOREM.md').read_text();assert '2^((n-3)/2)' in theorem
    tex=(HERE/'V68_SPINE_TREE_DAG_THEOREM.tex').read_text()
    for token in ('Spine tree--DAG separation','2^{k-1}','3k+4','does not identify'):assert token in tex
    boundary=(HERE/'PROOF_COMPLEXITY_BOUNDARY.md').read_text().lower();assert 'no simulation theorem' in boundary and 'tseitin' in boundary
    corpus='\n'.join(path.read_text().lower() for path in HERE.iterdir() if path.suffix in {'.md','.json','.tex'})
    forbidden=('p versus np is solved','we prove p != np','all affine-cell dags are polynomial','standard obdd lower bound follows','unrestricted nc0_3-avoid is solved')
