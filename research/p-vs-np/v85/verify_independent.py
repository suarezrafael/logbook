...
from distance_semiring import build_v75_model, distance_polynomial, remote_point_from_v75_model
...
def exact_coefficients(counts: dict[int, int], m: int, prefix: tuple[int, ...], radius: int) -> tuple[int, ...]:
    exact = [0] * (radius + 1)
    remaining = m - len(prefix)
    for output, multiplicity in counts.items():
        mismatch = sum(value != ((output >> i) & 1) for i, value in enumerate(prefix))
        for tail in range(remaining + 1):
            if mismatch + tail <= radius:
                exact[mismatch + tail] += multiplicity * comb(remaining, tail)
    return tuple(exact)


def verify_v75_semiring() -> tuple[int, int, int]:
    ...
    for case in range(8):
        ...
        for length in range(8):
            for value in range(min(1 << length, 4)):
                ...
                assert distance_polynomial(model, prefix, 2) == exact_coefficients(counts, 7, prefix, 2)
                coefficient_checks += 1
        target = int(remote_point_from_v75_model(model, 1)["target_integer"])
...
def main() -> None:
    ...
    truth = lambda terms: sum((sum((x & term) == term for term in terms) & 1) << x for x in range(8))
    truths = (truth((3, 4)), truth((3, 4)), truth((1, 2, 4)), truth((1, 2, 4)), truth((1, 2, 4)), truth((1, 2, 4)), truth((1, 2, 4)))
    assert all(sum(eval_gate(supports[i], truths[i], x) for i in range(4)) % 2 == 0 for x in range(64))
    for r in range(1, 7):
    ...
    for n, m, seed in REMOTE_CASES:
        ...
        image = [sum(eval_gate(triples[(seed * 17 + i * 7) % len(triples)], masks[i], x) << i for i in range(m)) for x in range(1 << n)]
        radius = max(r for r in range(m + 1) if (1 << n) * ball(m, r) < (1 << m))
        assert any(min((y ^ z).bit_count() for y in image) > radius for z in range(1 << m))

    assert verify_v75_semiring() == (8, 216, 8)
    print("V85 independent verification passed: Walsh census, C4 witness, remote existence, and 216 V75 distance coefficients.")
